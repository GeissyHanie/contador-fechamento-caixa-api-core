...
const insertDenomination = (req, res) => {
  ...
  res.status(200).json({ message: 'Quantidade registrada', type, quantity });
};

const getPartialResult = (req, res) => {
  const username = req.user.username;
  const userCounts = cashCounts[username] || {};
  let total = 0;
  const partials = denominations.map(type => {
  ...
  res.json({ partials, total });
};

module.exports = { insertDenomination, getPartialResult };
